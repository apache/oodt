...
public class MemoryBasedIngestMapper implements IngestMapper {
  ...
  private static Logger LOG = Logger.getLogger(MemoryBasedIngestMapper.class
      .getName());
  ...
  public synchronized TransactionId<?> getCatalogServiceTransactionId(
    ...
    LOG.log(Level.INFO,
        "Looking up CatalogService TransactionId for Catalog TransactionId '"
            + catalogTransactionId + "' and catalog '" + catalogId + "'");
   ...
   * 
   * @seeorg.apache.oodt.cas.catalog.repository.CatalogRepository#
   * getCatalogTransactionId (org.apache.oodt.cas.catalog.struct.TransactionId,
   * java.lang.String)
   */
   ...
   * (non-Javadoc)
   * 
   * @see org.apache.oodt.cas.catalog.mapping.IngestMapper#getPage(org.apache
  ...
  public synchronized Set<TransactionId<?>> getPageOfCatalogTransactionIds(
    ...
    List<CatalogReceipt> catalogReceipts = this.catalogIdToCatalogReceiptMapping
        .get(catalogId);
    if (catalogReceipts != null)
      for (int i = indexPager.getPageNum() * indexPager.getPageSize(); i < catalogReceipts
          .size()
          && i < (indexPager.getPageNum() + 1) * indexPager.getPageSize(); i++)
   ...
   * 
   * @see
   * org.apache.oodt.cas.catalog.mapping.IngestMapper#deleteAllMappingsForCatalog
   * (java.lang.String)
   */
  public synchronized void deleteAllMappingsForCatalog(String catalogId)
    ...
    if (catalogReceipts != null) {
      for (CatalogReceipt catalogReceipt : catalogReceipts) {
        TransactionIdMapping mapping = this.catalogInfoKeyMapping
  ...
  public synchronized void deleteAllMappingsForCatalogServiceTransactionId(
      ...
      throws CatalogRepositoryException {
    TransactionIdMapping mapping = this.catalogServiceTransactionIdKeyMapping
        .remove(catalogServiceTransactionId.toString());
    if (mapping != null)
      for (CatalogReceipt catalogReceipt : mapping.getCatalogReceipts()) {
        this.catalogIdToCatalogReceiptMapping
            .get(catalogReceipt.getCatalogId()).remove(catalogReceipt);
        this.catalogInfoKeyMapping.remove(generateKey(catalogReceipt
            .getTransactionId().toString(), catalogReceipt.getCatalogId()));
      }
  }
  ...
  public synchronized void deleteTransactionIdMapping(
    ...
    for (int i = 0; i < catalogReceipts.size(); i++) {
      if (catalogReceipts.get(i).getCatalogId().equals(catalogId)
          && catalogReceipts.get(i).getTransactionId().equals(
    ...
    TransactionIdMapping mapping = this.catalogInfoKeyMapping
        .remove(generateKey(catalogTransactionId.toString(), catalogId));
    this.catalogServiceTransactionIdKeyMapping.remove(mapping
        .getCatalogServiceTransactionId().toString());
  }
  ...
  public synchronized boolean hasCatalogServiceTransactionId(
      TransactionId<?> catalogServiceTransactionId)
      throws CatalogRepositoryException {
    return this.catalogServiceTransactionIdKeyMapping
        .containsKey(catalogServiceTransactionId.toString());
  }
  ...
  public synchronized void storeTransactionIdMapping(
    ...
    this.catalogServiceTransactionIdKeyMapping.put(catalogServiceTransactionId
        .toString(), mapping);
    this.catalogInfoKeyMapping
        .put(generateKey(catalogReceipt.getTransactionId().toString(),
            catalogReceipt.getCatalogId()), mapping);
    List<CatalogReceipt> catalogReceipts = this.catalogIdToCatalogReceiptMapping
        .get(catalogReceipt.getCatalogId());
    if (catalogReceipts == null)
   ...
   * (non-Javadoc)
   * 
   * @see org.apache.oodt.cas.catalog.repository.CatalogRepository#getCatalogs
   * (org.apache.oodt.cas.catalog.struct.TransactionId)
   */
  ...
  public CatalogReceipt getCatalogReceipt(
      ...
      throws CatalogRepositoryException {
    TransactionIdMapping mapping = this.catalogServiceTransactionIdKeyMapping
        .get(catalogServiceTransactionId);
    for (CatalogReceipt catalogReceipt : mapping.getCatalogReceipts())
  ...
  private static String generateKey(String catalogTransactionId,
      String catalogId) {
