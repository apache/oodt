...
public class XmlRpcWorkflowManager implements WorkflowManager {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(XmlRpcWorkflowManager.class);
  ...
  public Map getWorkflowInstanceById(String wInstId) {
    ...
    try {
      ...
      LOG.warn("Error obtaining workflow instance with ID: [" + wInstId + "]: Message: " + e.getMessage(), e);
  ...
  public List getWorkflowInstancesByStatus(String status)
    ...
    try {
      ...
      LOG.warn("Exception getting workflow instances by status: Message: ["+ e.getMessage() + "]", e);
  ...
  public List getWorkflowInstances() throws EngineException {
    ...
    try {
      ...
      LOG.warn("Exception getting workflow instances: Message: [" + e.getMessage() + "]", e);
      return workflowInstances;
    }
    ...
    if (wInsts != null && wInsts.size() > 0) {
      for (Object wInst1 : wInsts) {
        ...
        if (wInst.getWorkflow() == null || ((wInst.getWorkflow().getName() == null
          ...
          // repo
          try {
            LOG.info("Attempting to look up workflow [{}] in populate workflows", wInst.getWorkflow().getId());
            if (repo.getWorkflowById(wInst.getWorkflow().getId()) == null) {
              repo.addWorkflow(wInst.getWorkflow());
            }
          } catch (RepositoryException e) {
            LOG.warn("RepositoryException when looking up workflow [{}]: {}", wInst.getWorkflow().getId(), e.getMessage(), e);
          }
  ...
  private Workflow safeGetWorkflowById(String workflowId) {
    try {
      ...
      LOG.warn("Error getting workflow by its id [{}]: {}", workflowId, e.getMessage());
      return new Workflow();
    }
  }
