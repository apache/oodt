...
public class SSOProxy implements SSOMetKeys {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(SSOProxy.class);
  ...
  public IdentityDetails readIdentity(String username, String token)
    ...
    LOG.info("Obtaining identity: username: [{}]: token: [{}]: REST url: [{}]", username, token, IDENT_READ_ENDPOINT);
  ...
  public UserDetails getUserAttributes(String token) throws IOException, SingleSignOnException {
    HttpClient httpClient = new DefaultHttpClient();
    HttpPost post = new HttpPost(IDENT_READ_ENDPOINT);
    LOG.info("Obtaining user attributes: token: [{}]: REST url: [{}]", token, IDENT_ATTR_ENDPOINT);
  ...
  public void logout(String token) {
    ...
    LOG.info("Logging out: token: [{}]: REST url: [{}]", token, LOG_ENDPOINT);
    NameValuePair[] data = { new BasicNameValuePair("subjectid", token) };
  ...
  private IdentityDetails parseIdentityDetails(String serviceResponse) {
    ...
    try {
      ...
      LOG.warn("Error reading service response line: [{}]: {}", line, e.getMessage(), e);
    } finally {
      try {
        is.close();
