 ...
 * 
 *          <p>
 *          An implementation of a File {@link Catalog} using Apache's popular
 *          <a href="http://lucene.apache.org">Lucene</a> text indexing engine
 *          as a backend.
 *          </p>
 * 
 */
public class CuratorLuceneCatalog implements Catalog {
  ...
  /* our log stream */
  private static Logger LOG = Logger.getLogger(CuratorLuceneCatalog.class
      .getName());
   ...
   * @param pgSize
   *          The size of pages to be used when doing pagination of the catalog.
   * 
   * @param commitTimeout
   *          The amount of time (in seconds) that should be flowed down to the
   *          Lucene IndexReader and IndexWriters for their commit lock timeout
   *          property.
   * 
   * @param writeTimeout
   *          The amount of time (in seconds) that should be flowed down to the
   *          Lucene IndexWriters for their commit lock timeout property.
   * 
   * @param mergeFactor
   *          The merge factor to use when writing to the index.
   */
  public CuratorLuceneCatalog(String idxFilePath, ValidationLayer vLayer,
      int pgSize,
      long commitTimeout, long writeTimeout, int mergeFactor) {
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#addMetadata(gov.nasa.jpl.
   * oodt.cas.metadata.Metadata, gov.nasa.jpl.oodt.cas.filemgr.structs.Product)
   */
  public synchronized void addMetadata(Metadata m, Product product)
    ...
    if (p == null) {
      throw new CatalogException("Attempt to add metadata to a product: ["
          + product.getProductName() + "] that isn't the local cache!");
    } else {
      p.setMetadata(m);
      if (hasMetadataAndRefs(p)) {
        LOG.log(Level.FINE, "metadata and references present for product: ["
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#removeMetadata(gov.nasa.jpl
   * .oodt.cas.metadata.Metadata, gov.nasa.jpl.oodt.cas.filemgr.structs.Product)
   */
  public synchronized void removeMetadata(Metadata m, Product product)
    ...
    if (p == null) {
      // not in index
      throw new CatalogException("Attempt to remove metadata to a product: ["
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#addProduct(gov.nasa.jpl.oodt
   * .cas.filemgr.structs.Product)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#modifyProduct(gov.nasa.jpl
   * .oodt.cas.filemgr.structs.Product)
   */
  public synchronized void modifyProduct(Product product)
    ...
    if (CATALOG_CACHE.get(product.getProductId()) != null) {
      ...
      CompleteProduct cp = CATALOG_CACHE.get(product.getProductId());
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#removeProduct(gov.nasa.jpl
   * .oodt.cas.filemgr.structs.Product)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#setProductTransferStatus(
   * gov.nasa.jpl.oodt.cas.filemgr.structs.Product)
   */
  public synchronized void setProductTransferStatus(Product product)
      throws CatalogException {
    LOG.log(Level.FINE, "LuceneCatalog: seting product transfer status to: ["
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#addProductReferences(gov.
   * nasa.jpl.oodt.cas.filemgr.structs.Product)
   */
  public synchronized void addProductReferences(Product product)
    ...
    if (p == null) {
      throw new CatalogException("Attempt to add references to a product: ["
          + product.getProductName() + "] that isn't the local cache!");
    } else {
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#getProductById(java.lang.
   * String)
   */
  ...
  private CompleteProduct getCompleteProductById(String productId,
    ...
    try {
      ...
      CompleteProduct prod = toCompleteProduct(productDoc, getRefs, getMet);
      return prod;
    } catch (IOException e) {
      LOG.log(Level.WARNING, "IOException when opening index directory: ["
          + indexFilePath + "] for search: Message: " + e.getMessage());
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#getProductByName(java.lang
   * .String)
   */
  ...
  private Product getProductByName(String productName, boolean getRefs)
    ...
    try {
      ...
      if (hits.length() > 0) {
        ...
        LOG.log(Level.FINEST, "Request for product by name: [" + productName
            + "] returned no results");
        return null;
      }
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#getProductReferences(gov.
   * nasa.jpl.oodt.cas.filemgr.structs.Product)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#getProductsByProductType(
  ...
  private List<Product> getProductsByProductType(ProductType type,
      boolean getRefs) throws CatalogException {
    IndexSearcher searcher = null;
    List<Product> products = null;
  ...
  public Metadata getMetadata(Product product) throws CatalogException {
    ...
    try {
      ...
      // should be exactly 1 hit
      if (hits.length() != 1) {
        throw new CatalogException("Product: [" + product.getProductId()
  ...
  public Metadata getReducedMetadata(Product product, List<String> elements)
    ...
    for (int i = 0; i < elements.size(); i++) {
      ...
      reducedMetadata.addMetadata(element, fullMetadata.getMetadata(element));
    }
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#query(gov.nasa.jpl.oodt.cas
   * .filemgr.structs.Query, gov.nasa.jpl.oodt.cas.filemgr.structs.ProductType)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#getNumProducts(gov.nasa.jpl
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.util.Pagination#getFirstPage(gov.nasa.jpl
   * .oodt.cas.filemgr.structs.ProductType)
   */
  public ProductPage getFirstPage(ProductType type) {
    ...
    try {
      ...
      LOG.log(Level.WARNING,
          "CatalogException getting first page for product type: ["
              + type.getProductTypeId() + "] from catalog: Message: "
              + e.getMessage());
      return null;
    }
    ...
    if (products == null || (products != null && products.size() == 0)) {
      ...
      try {
        firstPage.setTotalPages(PaginationUtils.getTotalPage(getNumHits(query,
            type), pageSize));
      } catch (Exception e) {
        LOG.log(Level.WARNING, "Exception getting total pages for query: ["
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.util.Pagination#getLastProductPage(gov.nasa
   * .jpl.oodt.cas.filemgr.structs.ProductType)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.util.Pagination#getNextPage(gov.nasa.jpl.
   * oodt.cas.filemgr.structs.ProductType,
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.util.Pagination#getPrevPage(gov.nasa.jpl.
   * oodt.cas.filemgr.structs.ProductType,
   * gov.nasa.jpl.oodt.cas.filemgr.structs.ProductPage)
   */
   ...
   * 
   * @see
   * gov.nasa.jpl.oodt.cas.filemgr.catalog.Catalog#pagedQuery(gov.nasa.jpl.oodt
   * .cas.filemgr.structs.Query,
  ...
  public ProductPage pagedQuery(Query query, ProductType type, int pageNum)
    ...
    try {
      ...
      retPage.setTotalPages(PaginationUtils.getTotalPage(
  ...
  private synchronized void addCompleteProductToIndex(CompleteProduct cp)
    ...
    try {
      ...
      LOG.log(Level.WARNING, "Unable to index product: ["
          + cp.getProduct().getProductName() + "]: Message: " + e.getMessage(),
          e);
  ...
  private CompleteProduct toCompleteProduct(Document doc, boolean getRefs,
    ...
    if (getRefs) {
      ...
      if ((origRefs.length == dataStoreRefs.length)
        ...
        LOG.log(Level.WARNING, "Number of original refs: [" + origRefs.length
            + "] for product: [" + product.getProductName()
            + "] not equivalent to number of data store refs: ["
            + dataStoreRefs.length + "]: Skipping product references");
      }
    }
  ...
  private Document toDoc(Product product, Metadata metadata) {
    ...
    doc.add(new Field("product_id", product.getProductId(), Field.Store.YES,
    ...
    doc.add(new Field("product_structure", product.getProductStructure(),
        Field.Store.YES, Field.Index.UN_TOKENIZED));
    doc.add(new Field("product_transfer_status", product.getTransferStatus(),
        Field.Store.YES, Field.Index.UN_TOKENIZED));

    // product type
    doc.add(new Field("product_type_id", product.getProductType()
        .getProductTypeId(), Field.Store.YES, Field.Index.UN_TOKENIZED));
    doc.add(new Field("product_type_name", product.getProductType().getName(),
        Field.Store.YES, Field.Index.UN_TOKENIZED));
    doc.add(new Field("product_type_desc", product.getProductType()
        .getDescription() != null ? product.getProductType().getDescription()
        : "", Field.Store.YES, Field.Index.NO));
    doc.add(new Field("product_type_repoPath", product.getProductType()
        .getProductRepositoryPath() != null ? product.getProductType()
        .getProductRepositoryPath() : "", Field.Store.YES, Field.Index.NO));
    doc.add(new Field("product_type_versioner", product.getProductType()
        .getVersioner() != null ? product.getProductType().getVersioner() : "",
        Field.Store.YES, Field.Index.NO));
    ...
    for (Iterator<Element> i = elements.iterator(); i.hasNext();) {
      ...
      for (Iterator<String> j = values.iterator(); j.hasNext();) {
        String val = j.next();
        doc.add(new Field(key, val, Field.Store.YES, Field.Index.UN_TOKENIZED));
      }
    }
    ...
    for (Iterator<Reference> i = product.getProductReferences().iterator(); i
      ...
      doc.add(new Field("reference_orig", r.getOrigReference(),
          Field.Store.YES, Field.Index.NO));
      doc.add(new Field("reference_data_store", r.getDataStoreReference(),
          Field.Store.YES, Field.Index.NO));
      doc.add(new Field("reference_fileSize", String.valueOf(r.getFileSize()),
          Field.Store.YES, Field.Index.NO));
  ...
  private int getNumHits(Query query, ProductType type) throws CatalogException {
    IndexSearcher searcher = null;
    ...
    try {
      ...
      TermQuery prodTypeTermQuery = new TermQuery(new Term("product_type_id",
  ...
  private List<Product> paginateQuery(Query query, ProductType type, int pageNum)
    ...
    try {
      ...
      for (QueryCriteria queryCriteria : query.getCriteria())
        booleanQuery
            .add(this.getQuery(queryCriteria), BooleanClause.Occur.MUST);
      ...
      if (hits.length() > 0) {
        ...
        LOG.log(Level.WARNING, "Query: [" + query + "] for Product Type: ["
            + type.getProductTypeId() + "] returned no results");
      }
  ...
  private org.apache.lucene.search.Query getQuery(QueryCriteria queryCriteria)
      throws CatalogException {
    if (queryCriteria instanceof BooleanQueryCriteria) {
      ...
      case BooleanQueryCriteria.NOT:
        ...
        booleanQuery.add(new WildcardQuery(new Term(
            ((BooleanQueryCriteria) queryCriteria).getTerms().get(0)
                .getElementName(), "*")), BooleanClause.Occur.SHOULD);
        break;
      default:
  ...
  private List<Element> quietGetElements(ProductType type) {
    ...
    try {
      ...
      LOG.log(Level.WARNING, "Exception obtaining elements for product type: ["
