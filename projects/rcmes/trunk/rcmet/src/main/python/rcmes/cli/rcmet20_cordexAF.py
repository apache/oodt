...
def rcmet_cordexAF():
  ...
  import glob; import datetime; import time; import numpy as np; import numpy.ma as ma
  import do_data_prep_20; import do_metrics_20
  import rcmes.files_v12; import rcmes.process_v12
  import sys
  print 'Start RCMET'
  ...
  if GUI:
    ...
    #modelVarName='clt'
    mdlTimeStep='monthly'
    modelLatVarName='lat'; modelLonVarName='lon'; modelTimeVarName='time' # mdl var names for lat, long, & time coords
    workdir = '../cases/cordex-af/wrk1'; cachedir = '../cases/cordex-af/cache'; mdlDataDir = '/nas/share4-cf/jinwonki/data/cordex-af'
  if modelVarName=='pr':
    precipFlag = True
  else:
    precipFlag = False
  ...
  idObsDat=[]; idObsDatPara=[]; obsTimeStep=[]
  if GUI:
    ...
    numOBSs=2; idObsDat=[0,6]; idObsDatPara=[1,0]; obsTimeStep=['monthly','monthly']
  ...
  file_type='nc'; laName = modelLatVarName; loName = modelLonVarName
  latMin=ma.zeros(n_infiles); latMax=ma.zeros(n_infiles); lonMin=ma.zeros(n_infiles); lonMax=ma.zeros(n_infiles)
  for n in np.arange(n_infiles):
    ifile=FileList[n]
    status,latMin[n],latMax[n],lonMin[n],lonMax[n] = rcmes.files_v12.find_latlon_var_from_file(ifile,file_type,laName,loName)
  ...
  timeName=modelTimeVarName; mdldataTimeStep='monthly'; file_type='nc'
  n_mos=ma.zeros(n_infiles); newFileList=[]; mdlStartT=[]; mdlEndT=[]; mdlName=[]; k=0
  for n in np.arange(n_infiles):
    ...
    modelTimes = rcmes.process_v12.decode_model_timesK(ifile,timeName,file_type)
    ...
    print 'Mdl Times decoded: n= ',n,' Name: ',mdlName[n],' length= ',len(modelTimes),' 1st mdl time: ',mdlStartT[n].strftime("%Y/%m"), \
                                                                                      ' Lst mdl time: ',mdlEndT[n].strftime("%Y/%m")
  ...
  for n in np.arange(n_infiles):
    ...
    yy=int(startTime[n].strftime("%Y")); mm=int(startTime[n].strftime("%m"))
    if mm != 1:
      yy=yy+1; mm=1
    startTime[n]=datetime.datetime(int(yy),int(mm),1,0,0,0,0)
    yy=int(endTime[n].strftime("%Y")); mm=int(endTime[n].strftime("%m"))
    if mm != 12:
      yy=yy-1; mm=12
  ...
  if GUI:
    ...
    choice = 3
  if choice==1:
    startTime=maxAnlT0; endTime=maxAnlT1; print 'Maximum(model,obs) period is selected. Some models will be dropped from evaluation'
  if choice==2:
    startTime=minAnlT0; endTime=minAnlT1; print 'Minimum(model,obs) period is selected. All models will be evaluated except there are problems'
  if choice==3:
    ...
    startTime=datetime.datetime(startYear,1,1,0,0); endTime=datetime.datetime(endYear,12,31,0,0)
  ...
  k=0; newFileList=[]; name=[]
  print 'n_infiles= ',n_infiles
  for n in np.arange(n_infiles): 
    ifile=FileList[n]; nMos=n_mos[n]
    print mdlName[n],n_mos[n],mdlStartT[n],startTime,mdlEndT[n],endTime
    if (mdlStartT[n]<=startTime) & (mdlEndT[n]>=endTime):
      newFileList.append(ifile)
      name.append(mdlName[n])
      k += 1
  FileList=newFileList; newFileList=0; FileList.sort(); print 'the number of select files = ',len(FileList)
  mdlName=name
  ...
  naLons=1; naLats=1; dLon = 0.5; dLat = 0.5  # these are dummies for regridOption = 1 & 2
  if GUI:
  ...
  if regridOption==0: regridOption = 'obs'
  if regridOption==1: regridOption = 'model'
  ...
  if regridOption==2:
    ...
    dLon = 0.44; dLat = 0.44; lonMin = -24.64; lonMax = 60.28; latMin = -45.76; latMax = 42.24
  ...
  if GUI:
    if regridOption==2:
      ...
      nLons=int((lonMax-lonMin+1.e-5*dLon)/dLon)+1; nLats=int((latMax-latMin+1.e-5*dLat)/dLat)+1
  ...
  if timeRegridOption==0: timeRegridOption='mean over all times: i.e., annual-mean climatology'
  if timeRegridOption==1: timeRegridOption='annual'
  if timeRegridOption==2: timeRegridOption='monthly'
  if timeRegridOption==3: timeRegridOption='daily'
  print 'timeRegridOption= ',timeRegridOption
   ...
   #******************************************************************************************************************
  numSubRgn=21
  subRgnLon0=ma.zeros(numSubRgn); subRgnLon1=ma.zeros(numSubRgn)
  subRgnLat0=ma.zeros(numSubRgn); subRgnLat1=ma.zeros(numSubRgn)
  ...
  maskOption = 0; maskLonMin = 0; maskLonMax = 0; maskLatMin = 0; maskLatMax = 0; rgnSelect=0
  ...
  if choice == 'y':
     ...
     if maskInputChoice == 1:
       ...
       rgnSelect=3
       if(GUI): rgnSelect=raw_input('Select the region for which regional-mean timeseries are to be analyzed\n')
     #if maskInputChoice==0:    # Read mask from file
  ...
  if choice==1: metricOption = 'mae'
  if choice==2: metricOption = 'difference'
  if choice==3: metricOption = 'acc'
  if choice==4: metricOption = 'patcor'
  if choice==5: metricOption = 'pdf'
  if choice==6: metricOption = 'rms'
  ...
  ###################################################################################################
  modifyPlotOptions = 'no'; plotTitle = modelVarName + '_'; plotFilenameStub = modelVarName + '_'
  if GUI:
  ...
  numOBS,numMDL,nT,ngrdY,ngrdX,Times,obsData,mdlData,obsRgn,mdlRgn,obsList,mdlList = do_data_prep_20.prep_data(           \
         cachedir,workdir,                                                                                                \
  ...
  print '-----------------------------------------------'; print 'mdlID  numMOs  mdlStartTime mdlEndTime fileName'; print '-----------------------------------------------'
  ...
  do_metrics_20.metrics_plots(numOBS,numMDL,nT,ngrdY,ngrdX,Times,obsData,mdlData,obsRgn,mdlRgn,obsList,mdlList,      \
                              workdir,                                                                               \
                              mdlSelect,obsSelect,                                                                   \
