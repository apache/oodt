...
def do_regrid(q,lat,lon,lat2,lon2,order=1,mdi=-999999999):
   ...
   nlat  = q.shape[0]   ; nlon  = q.shape[1]
   nlat2 = lat2.shape[0]; nlon2 = lon2.shape[1]
    # To make our lives easier down the road, let's turn these into arrays of x & y coords
   loni = lon2.ravel(); lati = lat2.ravel()
    # NB. it won't run unless you do this
   loni = loni.copy(); lati = lati.copy()
...
def calc_average_on_new_time_unit_K(data,dateList,unit):
   ...
   #   Peter Lean August 2010
   #################################################################################################
   import datetime; import numpy; import numpy.ma as ma
   ...
   if unit=='full':
      ...
      timeunits = numpy.array(timeunits, dtype=int)
   #print 'timeunits in temporal regridding corresponding to: ',unit; print timeunits
...
def decode_model_timesK(ifile,timeVarName,file_type):
   ...
   #     Peter Lean February 2011
   #################################################################################################
   import datetime; import re; import string; import math; import numpy; import Nio
   ...
   units = ''
   try:
     mysearch = re.search('minutes',timeFormat).end(); units = 'minutes'
   except:
     pass
   try:
     mysearch = re.search('hours',timeFormat).end(); units = 'hours'
   except:
     pass
   try:
     mysearch = re.search('days',timeFormat).end(); units = 'days'
   except:
     pass
   try:
     mysearch = re.search('months',timeFormat).end(); units = 'months'
   except:
     pass
   try:
     mysearch = re.search('years',timeFormat).end(); units = 'years'
   except:
     pass
    # cut out base time (the bit following 'since')
   base_time_string = string.lstrip(timeFormat[sinceLoc:])
