...
public class ProductPageResourceTest {
    ...
    @Test
    public void testXmlMarshalling() throws JAXBException, IOException, SAXException, MimeTypeException {
        ...
        ProductPage productPage = new ProductPage(1, 1, 20,
        pageProducts);

//      Create a ProductPageResource using Reference, Metadata and Product instances.
        ProductPageResource resource = new ProductPageResource(productPage,metadataList,productReferencesList,new File("/tmp"));

//      Generate the expected output.
        String expectedXml =
                "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
                        + "<productPage>"
                        + "<pageSize>" + resource.getPageSize() + "</pageSize>"
                        + "<pageNum>" + resource.getPageNum() + "</pageNum>"
                        + "<totalPages>" + resource.getTotalPages() + "</totalPages>"
                        + "<numOfHits>" + resource.getNumOfHits() + "</numOfHits>"
                        +"<products>"
                        + "<product>"
                        ...
                        + "<transferStatus>"+product.getTransferStatus()+"</transferStatus>"
                        + "<metadata>"
                        + "<keyval>"
                        + "<key>" + metadataList.get(0).getAllKeys().get(0) + "</key>"
                        + "<val>" + metadataList.get(0).getAllValues().get(0) + "</val>"
                        + "</keyval>"
                        + "</metadata>"
                        + "<references>"
                        + "<reference>"
                        ...
                        + "</originalReference>"
                        + "<mimeType>" + productReferencesList.get(0).get(0).getMimeType().getName() + "</mimeType>"
                        + "<fileSize>" + productReferencesList.get(0).get(0).getFileSize() + "</fileSize>"
                        + "</reference>"
                        + "</references>"
                        + "</product>"
                        +"</products>"
