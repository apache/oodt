...
//
// $Id: Configuration.java,v 1.8 2004-06-16 22:15:26 kelly Exp $
...
public class Configuration {
	...
	public static final File ALT_LIB_CONFIG_FILE = new File(System.getProperty("java.home", "/") + File.separator + ".."
	  ...
	  * the system property <code>jpl.eda.Configuration.url</code> or the file in the
	 ...
	 public static Configuration getConfiguration() throws IOException, SAXException, MalformedURLException {
		 ...
		 // First preference: URL via the jpl.eda.Configuration.url prop.
		 String urlString = System.getProperty("jpl.eda.Configuration.url");
		 if (urlString != null) {
			 url = new URL(urlString);
		 } else {
			 File file = null;

			 // Second preference: file via the jpl.eda.Configuration.file prop.
			 String filename = System.getProperty("jpl.eda.Configuration.file");
			 if (filename != null) {
	...
	Configuration(InputSource inputSource) throws IOException, SAXException {
		...
		for (int eachChild = 0; eachChild < list.getLength(); ++eachChild) {
			...
			if (childNode.getNodeName().equals("webServer")) {
				...
				properties.setProperty("jpl.eda.Configuration.webServer.baseURL", getWebServerBaseURL());
				...
				for (int i = 0; i < children.getLength(); ++i) {
					...
					esc.getProperties().setProperty("jpl.eda.Configuration.url", inputSource.getSystemId());
					execServers.add(esc);
				}
			}
		}
	...
	private void initializeContext() {
		contextEnvironment.put(Context.INITIAL_CONTEXT_FACTORY, "jpl.eda.object.jndi.ObjectCtxFactory");
		String registryList = System.getProperty("jpl.eda.rmiregistries", System.getProperty("rmiregistries"));
	...
	public static final String WEB_PROTOCOL_PROPERTY = "jpl.eda.Configuration.webProtocol";
