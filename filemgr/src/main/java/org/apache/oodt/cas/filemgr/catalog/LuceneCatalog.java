...
public class LuceneCatalog implements Catalog {
    ...
    public synchronized void addMetadata(Metadata m, Product product)
        ...
        if (hasMetadataAndRefs(p)) {
            LOG.info("metadata and references present for product: [{}]", product.getProductId());
    ...
    public synchronized void removeMetadata(Metadata m, Product product)
        ...
        if (valLayer!=null) {
	        try {
	            ...
	            String msg = String.format("ValidationLayerException when trying to obtain element list for product type: %s: %s", product.getProductType().getName(), e.getMessage());
                LOG.error(msg, e);
    ...
    public synchronized void modifyProduct(Product product)
            throws CatalogException {
        if (product.getProductId()!=null && CATALOG_CACHE.containsKey(product.getProductId())) {
            LOG.info("Modifying product: [{}]: product found in cache!", product.getProductId());
    ...
    public synchronized void setProductTransferStatus(Product product)
            throws CatalogException {
        LOG.info("LuceneCatalog: setting product transfer status to: [{}] for product: [{}]", product.getTransferStatus(), product.getProductId());
    ...
    private CompleteProduct getCompleteProductById(String productId,
        ...
        try {
            ...
            LOG.warn("IOException when opening index directory: [{}] for search: {}", indexFilePath, e.getMessage(), e);
        ...
        try {
            ...
            if(check.totalHits>0) {
                ...
                if (hits.length > 0) {
                    ...
                    LOG.info("Request for product by name: [{}] returned no results", productName);
                    return null;
                }
            }
            else{
        ...
        try {
            ...
            if(check.totalHits>0) {
                ...
                if (hits.length > 0) {
                    ...
                    LOG.info("Request for products returned no results");
                    return null;
                }
            }
        } catch (IOException e) {
        ...
        try {
            ...
            if(check.totalHits>0) {
                ...
                if (hits.length > 0) {
                    ...
                    LOG.info("Request for products by type: [{}] returned no results", type.getProductTypeId());
                    return null;
                }
            }
...
//TODO CLOSE
    ...
    public synchronized List<Product> getTopNProducts(int n) throws CatalogException {
        ...
        try {
            ...
            Sort sort = new Sort(new SortField("CAS.ProductReceivedTime",
                    SortField.Type.STRING, true));
            LOG.info("Querying LuceneCatalog: [{}]", booleanQuery);
            //TODO FIX NUMBER OF RECORDS
    ...
    public ProductPage getFirstPage(ProductType type) {
        ...
        try {
          products = paginateQuery(query, type, 1, firstPage);
            ...
            LOG.warn("CatalogException getting first page for product type: [{}] from catalog: {}", type.getProductTypeId(), e.getMessage(), e);
            return null;
        }
    ...
    public ProductPage getLastProductPage(ProductType type) {
        ...
        try {
            products = paginateQuery(query, type, firstPage.getTotalPages(), lastPage);
          	...
          	LOG.warn("CatalogException getting last page for product type: [{}] from catalog: {}",type.getProductTypeId(), e.getMessage(), e);
          	return null;
        }
    ...
    public ProductPage getNextPage(ProductType type, ProductPage currentPage) {
        ...
        try {
            ...
            LOG.warn("CatalogException getting next page for product type: [{}] from catalog: {}", type.getProductTypeId(), e.getMessage(), e);
            return ProductPage.blankPage();
        }
    ...
    public ProductPage getPrevPage(ProductType type, ProductPage currentPage) {
        ...
        try {
            ...
            LOG.warn("CatalogException getting prev page for product type: [{}] from catalog: {}", type.getProductTypeId(), e.getMessage(), e);
            return null;
        }
    ...
    public ProductPage pagedQuery(Query query, ProductType type, int pageNum)
        ...
        try {
            ...
            LOG.warn("CatalogException when doing paged product query: {}", e.getMessage(), e);
            throw new CatalogException(e.getMessage(), e);
        }
    ...
    private synchronized void removeProductDocument(Product product)
        ...
        try {
            LOG.info("LuceneCatalog: remove document from index for product: [{}]", product.getProductId());
            ...
            LOG.error("Exception removing product: [{}] from index: {}", product.getProductName(), e.getMessage(), e);
            throw new CatalogException(e.getMessage(), e);
        } finally {
            if (reader != null) {
                try {
    ...
    private synchronized void addCompleteProductToIndex(CompleteProduct cp)
        ...
        try {
            ...
            String msg = String.format("Unable to index product: [%s]: %s",cp.getProduct().getProductName(), e.getMessage());
            LOG.warn(msg, e);
            throw new CatalogException(msg, e);
        } finally {
            try {
    ...
    private CompleteProduct toCompleteProduct(Document doc, boolean getRefs,
        ...
        if (getMetadata) {
            ...
            if (valLayer!=null) {
	            ...
	            try {
	                ...
	                LOG.warn("Unable to obtain metadata for product: [{}]: {}", product.getProductName(), e.getMessage(), e);
	            }
            } else {
        ...
        if (getRefs) {
            ...
            if ((origRefs.length == dataStoreRefs.length)
                ...
                LOG.warn("Number of original refs ({}) for product: [{}] not equal to number of datastore refs ({}). Skipping product refs", origRefs.length, product.getProductName(), dataStoreRefs.length);
            }
        }
...
//TODO CHECK STORED TYPES
        ...
        for (String key : keys) {
          List<String> values = metadata.getAllMetadata(key);
            ...
            if (values == null) {
                LOG.warn("No metadata specified for product: [{}] for required field: [{}]. Attempting to continue processing metadata", product.getProductName(), key);
                continue;
            }
    ...
    private int getNumHits(Query query, ProductType type)
        ...
        try {
            ...
            String msg = String.format("IOException when opening index directory: [%s] for search: %s", indexFilePath, e.getMessage());
            LOG.warn(msg, e);
            throw new CatalogException(msg, e);
        } finally {
            if (searcher != null) {
        ...
        try {
            ...
            if(check.totalHits>0) {
                ...
                if (hits.length > 0) {
                    ...
                    LOG.warn("Query: [{}] for product type: [{}] returned no results", query, type.getProductTypeId());
                }
            }
    ...
    private List<Element> quietGetElements(ProductType type) {
        ...
        try {
            ...
            LOG.warn("Exception obtaining elements for product type: [{}]: {}", type.getName(), e.getMessage(), e);
        }
