 ...
 * 
 */
public class LuceneCatalog extends AbstractCatalog {
    ...
    public synchronized void modifyProduct(Product product)
        ...
        if (CATALOG_CACHE.get(product.getProductId()) != null) {
            ...
            // we don't lose it
            CompleteProduct completeProduct = this.getCompleteProductById(product.getProductId(), true, true);
            ...
            try {
            	...
            	throw new CatalogException("Failure occured during product removal for replacement -- reverting back : " + e.getMessage());
            }
    ...
    public synchronized void setProductTransferStatus(Product product)
        ...
        if (CATALOG_CACHE.get(product.getProductId()) != null) {
            ...
            CompleteProduct cp = CATALOG_CACHE.get(product
                    .getProductId());
            cp.getProduct().setTransferStatus(product.getTransferStatus());
        } else {
            // need to grab the metadata for the existing product, and make sure
            // we don't lose it
            CompleteProduct completeProduct = this.getCompleteProductById(product.getProductId(), true, true);
            ...
            try {
            	// now remove the product's document from the catalog
            	removeProductDocument(product);
            	completeProduct.getProduct().setTransferStatus(product.getTransferStatus());
                addCompleteProductToIndex(completeProduct);
            }catch (Exception e) {
            	throw new CatalogException("Failure occured during product removal for replacement -- reverting back : " + e.getMessage());
            }
    ...
    private CompleteProduct getCompleteProductById(String productId,
        ...
        CompleteProduct p = CATALOG_CACHE.get(productId);
        if (p != null) {
        	return p;
        }else {
    ...
    public List<Metadata> getReducedMetadata(Query query, ProductType type, List<String> elements)
        ...
        List<CompleteProduct> fullProducts = this._paginateQuery(query, type, -1).pageOfProducts;
        List<Metadata> metadatas = new Vector<Metadata>();

        if (fullProducts != null && fullProducts.size() > 0) {
            for (CompleteProduct cp : fullProducts) {
            	Metadata m = new Metadata();
            	Metadata cpMet = cp.getMetadata();
            	if (elements != null) {
	            	for (String element : elements) {
	            		String val = cpMet.getMetadata(element);
	            		if (val != null)
    ...
    public List<String> query(Query query, ProductType type) throws CatalogException {
        // paginate products returns full products, but the query method
        // is expected to return product ids
        List<Product> fullProducts = paginateQuery(query, type, -1).getPageProducts();
    ...
    public List<Product> getTopNProducts(int n) throws CatalogException {
        ...
        try {
            ...
            if (hits.length() > 0) {
                int i = 0;
                while (products.size() < n && i < hits.length()) {
                    Document productDoc = hits.doc(i);
    ...
    public List<Product> getTopNProducts(int n, ProductType type)
        ...
        for (int pageNum = 1; pageNum < numPages + 1; pageNum++) {
            ProductPage page = paginateQuery(query, type, pageNum);
...
//        Query query = new Query();
        ...
        try {
            return paginateQuery(new Query(), type, 1);
...
//        Query query = new Query();
        ...
        try {
            return paginateQuery(new Query(), type, firstPage.getTotalPages());
...
//            lastPage.setNumOfHits(numOfHits);
//        }
//
//        return lastPage;
    }
...
//        Query query = new Query();
        ...
        try {
            return paginateQuery(new Query(), type, currentPage.getPageNum() + 1);
...
//            return retPage;
            return paginateQuery(query, type, pageNum);
        } catch (Exception e) {
            e.printStackTrace();
            LOG.log(Level.WARNING,
    ...
    private class LuceneProductPage {
    	int numOfHits;
    	List<CompleteProduct> pageOfProducts;
    	public LuceneProductPage(List<CompleteProduct> pageOfProducts, int numOfHits) {
    		this.pageOfProducts = pageOfProducts;
    		this.numOfHits = numOfHits;
    	}
    }
    
    private LuceneProductPage _paginateQuery(Query query, ProductType type, int pageNum) throws CatalogException {
    	return this._paginateQuery(query, type, pageNum, true, true);
	}
    
    private LuceneProductPage _paginateQuery(Query query, ProductType type, int pageNum, boolean getMet, boolean getRefs)
    		throws CatalogException {
        List<CompleteProduct> products = new Vector<CompleteProduct>();
        IndexSearcher searcher = null;
        ...
        try {
            ...
            if (hits.length() > 0) {
                ...
                if (doSkip) {
                    ...
                    for (int i = startNum; i < Math.min(hits.length(),
                            (startNum + pageSize)); i++) {
                        Document productDoc = hits.doc(i);
                        products.add(toCompleteProduct(productDoc,
                        		getMet, getRefs));
                    }
                } else {
                    for (int i = 0; i < hits.length(); i++) {
            ...
            return new LuceneProductPage(products, hits.length());
        } catch (Exception e) {
            e.printStackTrace();
            LOG.log(Level.WARNING,
                    "IOException when opening index directory: ["
    ...
    private ProductPage paginateQuery(Query query, ProductType type, int pageNum)
            throws CatalogException {
    	LuceneProductPage page = this._paginateQuery(query, type, pageNum, false, false);
    	ProductPage productPage = new ProductPage();
    	productPage.setNumOfHits(page.numOfHits);
    	productPage.setPageNum(pageNum);
    	productPage.setPageSize(this.pageSize);
    	Vector<Product> products = new Vector<Product>();
    	for (CompleteProduct cp : page.pageOfProducts)
    		products.add(cp.getProduct());
    	productPage.setPageProducts(products);
    	return productPage;
    }
    
    private org.apache.lucene.search.Query getQuery(QueryCriteria queryCriteria) throws CatalogException {
    	return this.getQuery(queryCriteria, false);
    }

    private org.apache.lucene.search.Query getQuery(QueryCriteria queryCriteria, boolean negated) throws CatalogException {
        if (queryCriteria instanceof BooleanQueryCriteria) {
            ...
            case BooleanQueryCriteria.NOT:
            	//return this.getQuery(((BooleanQueryCriteria) queryCriteria).getTerms().get(0), true);
            ...
            for (QueryCriteria qc : ((BooleanQueryCriteria) queryCriteria).getTerms())
                booleanQuery.add(this.getQuery(qc, negated), occur);
            
            return booleanQuery;
        } else if (queryCriteria instanceof TermQueryCriteria) {
            String val = ((TermQueryCriteria) queryCriteria).getValue();
            TermQuery termQuery = new TermQuery(new Term(queryCriteria.getElementName(), val));
//            if (negated) {
//                BooleanQuery booleanQuery = new BooleanQuery();
//                booleanQuery.add(new WildcardQuery(new Term(queryCriteria.getElementName(), 
...
//            }
            ...
            RangeQuery rangeQuery = new RangeQuery(startTerm, endTerm, inclusive);
//            if (negated) {
//                BooleanQuery booleanQuery = new BooleanQuery();
//                booleanQuery.add(new WildcardQuery(new Term(queryCriteria.getElementName(), 
//                		"*")), BooleanClause.Occur.SHOULD);
